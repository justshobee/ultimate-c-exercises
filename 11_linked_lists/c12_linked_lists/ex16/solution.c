...
/*                                                                            */
/*   Created: 2026/08/28 15:52:58 by shobeedev // learn the why,              */
/*   Updated: 2026/08/28 16:51:57 by shobeedev // not only the how.           */
/*                                                                            */
/* ************************************************************************** */
...
void ft_sorted_list_insert(t_list **head, void *data,  int (*cmp)(void*, void*))
{
	t_list	*tmp;
	t_list	*ptr;
	t_list	*prev;
...
void	test_int()
{
	...
	*data = rand() % (CHAR_MAX + 1);
	...
	ft_sorted_list_insert(&head, data, ft_ncmp);
	print_lst(head, 'i');
	free_lst(head, 1);
}
...
void	test_str(int size, char **str)
{
	...
	ft_sorted_list_insert(&head, str[0], ft_strcmp);
	print_lst(head, 's');
	free_lst(head, 0);
}
